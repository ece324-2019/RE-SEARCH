...
def eval(model, loader, loss_fnc, optimizer= None, train=False,cfm = False):
    ...
    for i, data in enumerate(loader):
        ...
        if args.type != 'buttons':
            ...
            output_labels = torch.tensor(np.around(old_outputs.detach().numpy()))
...
def dab(args, trainloader, validloader, testloader,name):
    ...
    torch.save(model,'model_'+str(name) + '_' + str(round(test_acc,3)) +'.pt')
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(args.plot_folder+'acc_' + str(name) + '.jpg')
    ...
    plt.ylabel('Accuracy')
    plt.legend()
    plt.savefig(args.plot_folder+'acc_filtered_' + str(name) + '.jpg')
    plt.close()
    ...
    # plt.ylabel('Loss')
    # plt.legend()
    # plt.savefig('../plots/'+'loss_filtered_' + name + '.jpg')
    #
...
if __name__ == '__main__':
    ...
    args.plot_folder = "../plots/"
    data_folder = '../button_dataset'
