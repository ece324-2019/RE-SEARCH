...
""" FINAL MODELS """
class cnn_sleeves(nn.Module):
    def __init__(self, num_class = 3, batch_norm=True, dropout=0.0, n1=5, n2=8, k1=3, k2=5, n3=5, k3=3,k4=3,n4=3, l1=40):
        super(cnn_sleeves, self).__init__()
...
class baseline(nn.Module):
    def __init__(self, num_class,type='not buttons'):

        super(baseline, self).__init__()
        self.type = type
        self.fc1 = nn.Linear(100*100,40*40)
        self.fc2 = nn.Linear(40*40, num_class)
...
class cnn_colors(nn.Module):

    def __init__(self, num_class =7, batch_norm = True, dropout = 0.0, n1 = 5, n2 = 8, k1 = 3, k2 = 5, l1= 40, type = 'colors'):
        super(cnn_colors,self).__init__()
        self.L = int((100 - (k1 - k1 % 2)) / 2)
        self.L = int((self.L - (k2 - k2 % 2)) / 2)
    ...
    def forward(self, x, batch_norm=True, loss_fnc = 'MSE'):
        x = self.pool(F.relu(self.conv_bn1(self.dropout2(self.conv1(x)))))
        x = self.pool(F.relu(self.conv_bn2(self.dropout2(self.conv2(x)))))
...
class cnn_neckline(nn.Module):
    def __init__(self, num_class, batch_norm=True, dropout=0.0, n1=5, n2=5, k1=3, k2=5,n3 = 3,k3=5, l1=40, type='nobutton'):
        super(cnn_neckline, self).__init__()
        self.type = type
        if type == 'buttons':
            self.L = int((100 - (k1 - k1 % 2)) / 2)
        else:
...
class cnn_buttons(nn.Module):
    ...
    def __init__(self, num_class, batch_norm =False, dropout = 0.0, n1 = 10, n2 = 10, k1 = 2, k2 = 3, l1= 40, type = 'colors'):
