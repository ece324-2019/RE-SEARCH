...
class baseline(nn.Module):
    def __init__(self, num_class, batch_norm =False, dropout = 0.0,type = 'nobuttons'):

        super(baseline, self).__init__()
        # self.fc1 = nn.Linear(100*100,40*40)
        self.type = type
        if self.type == 'buttons':
            self.fc2 = nn.Linear(256*256, num_class)
        else:
...
class cnn5(nn.Module):
    def __init__(self, num_class, batch_norm =False,dropout = 0.0):
        ...
        self.conv1 = nn.Conv2d(3,5,2)
        self.conv2 = nn.Conv2d(5,5,2)
        self.conv3 = nn.Conv2d(5,5,2)
        self.conv4 = nn.Conv2d(5,5,2)
        self.conv5 = nn.Conv2d(5,8,2)
        ...
        if batch_norm:
            ...
            self.conv_bn4 = nn.BatchNorm2d(5)
            self.conv_bn5 = nn.BatchNorm2d(8)
    ...
    def forward(self, x, batch_norm=False, loss_fnc = 'MSE'):
        if (batch_norm):
            ...
            x = self.pool(F.relu(self.conv_bn3(self.dropout2(self.conv3(x)))))
            x = self.pool(F.relu(self.conv_bn4(self.dropout2(self.conv4(x)))))
            x = self.pool(F.relu(self.conv_bn5(self.dropout2(self.conv5(x)))))
            x = x.view(-1, 8*2*2)
            x = F.relu(self.bn1(self.dropout1(self.fc1(x))))
            x = self.bn2(self.fc2(x))
            if loss_fnc == 'MSE' and self.type != 'buttons':
            ...
            x = self.pool(F.relu(self.dropout2(self.conv3(x))))
            x = self.pool(F.relu(self.dropout2(self.conv4(x))))
            x = self.pool(F.relu(self.dropout2(self.conv5(x))))
            x = x.view(-1, 8*2*2)
            x = F.relu(self.dropout1(self.fc1(x)))
            x = self.fc2(x)
            if loss_fnc == 'MSE' and self.type != 'buttons':
