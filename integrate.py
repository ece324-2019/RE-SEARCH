...
def integration(test_folder,in_type):
    ...
    class_n = ["collar", "crew", "square", "turtle", "v-neck"]
    class_s = ["long", "short", "sleeveless"]
    class_b = ['buttons','no buttons']

    device = "cpu" # torch.device("cuda: 0" if torch.cuda.is_available() else "cpu")
    if in_type == 'colors':
        model = torch.load('./models/model_c1.pt')
        model.eval()
        class_names = class_c
        norm = 0.69370186
        std = 0.3048072
        ...
        device = "cpu"
        model = torch.load('./models/model_s.pt')
        model.eval()
        class_names = class_s
        norm = 0.69370186
        std = 0.3048072
    elif in_type == 'necklines':
        model = torch.load('../RE-SEARCH/model_n.pt')
        model.eval()
        class_names = class_n
        norm = 0.68, 0.68, 0.68
        ...
        device = "cpu"
        model = torch.load('../RE-SEARCH/model_b.pt')
        model.eval()
        class_names = class_b
        norm = 0.69370186
        std = 0.3048072
    ...
    for i, filename in enumerate(glob.glob(test_folder + '/*.jpg')):
        ...
        if (in_type == 'colors' and value < 0.80) or (in_type == 'sleeves' and value < 0.1) or (in_type == 'necklines' and value < 0.65) or (in_type == 'buttons' and value < 0.80):
            value = 'NULL'
        else:
            value = round(value,3)
        out += [[label,value]]
